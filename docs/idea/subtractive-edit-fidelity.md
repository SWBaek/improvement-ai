...
전달 형태는 Skill보다 always-on 지침에 가깝다. 트리거가 있는 작업이 아니라, 규칙·문서·코드를 고치는 모든 세션에서 나타나기 때문이다. 다만 지금 루트 `AGENTS.md`에 문장을 넣는 것은 아직 가설이다. 코드 벤치마크에서 “workaround 하지 마라”는 거의 효과가 없었고, 부정 지시는 바로 이 실패 모드를 재현할 수 있다. 남는 연구 질문은 원인 보강이 아니라, **결과 계약 한두 문장**이 정책 파일에서 레버가 되는지다.
...
- 효과가 있어도 이 저장소 `AGENTS.md`에는 정체성·불변·승인 경계만 둔다는 기존 예산을 깨지 않게, 한 줄 이내로만 검토한다. 대상 프로젝트 `AGENTS.md`는 별 표면이다.
- 효과가 없거나 초과 삭제가 늘면 Skill이나 Blueprint로 올리지 않는다.
