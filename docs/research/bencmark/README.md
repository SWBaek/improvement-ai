# Benchmark Research

`docs/research/bencmark/`는 Capability Blueprint 전략과 비교할 외부 프로젝트, 패턴과 구현 사례의 조사 기록을 보존한다. 이 문서는 Blueprint 계약이나 채택된 ADR이 아니며, 조사 시점의 공개 자료를 근거로 이후 설계·Pilot 판단에 사용할 evidence를 제공한다.

## Reports

| Report | 조사 범위 |
|---|---|
| [Karpathy LLM Wiki ecosystem and Blueprint strategy](karpathy-llm-wiki-ecosystem.md) | `llm-wiki.md` 이후 Idea File, Skill, framework, CLI, Plugin과 App 구현을 비교하고 `improvement-ai`의 Blueprint-only 전략을 평가 |
