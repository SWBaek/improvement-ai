...
- enter Continuity before durable project work begins while leaving general questions, readily re-derived read-only checks, and atomic meaning-preserving edits outside the loop;
- support multiple open Work Items and concurrent Agent sessions, each with a human-selected session-scoped Focus after a compact overview and recommendation;
...
- Never select the Session Focus on the human's behalf; recommendations do not change Work Item priority or status. A request that unambiguously names an existing stable Work Item may select it for that session, but a semantic match inferred by the Agent may only be recommended.
- Never begin durable project mutation without running Brief and establishing a committed Work Item as the current session's human-selected Focus. When none fits, remain read-only, present update, candidate, consolidation, and creation options, and wait for the required creation, external-write, and Focus authority.
...
- Keep exactly one authoritative Handoff source or location. It must explicitly represent either no current checkpoint or one current human-confirmed checkpoint; absence, an actually blank document, and placeholder content are not empty-state representations. A checkpoint transfers one selected session's Focus and does not replace or cancel other active Work Items or concurrent sessions. Do not create, replace, or clear a checkpoint canonically without an explicit human request or confirmation, and do not create a session-by-session Handoff archive by default.
...
| Session Focus | The human-selected Work Item for one current Agent session. It is transient bounded context, not a project-wide lock, single active objective, Agent registry, or second durable task record. Concurrent sessions may select different committed Work Items. |
...
| Handoff | One authoritative location that explicitly represents either no current checkpoint or the latest human-confirmed resume checkpoint for one selected transfer. A checkpoint contains that session's Focus, last verified state, exact next action, blockers or unknowns, freshness evidence, and minimal authoritative references; other active work remains recoverable from its Work Items. |
...
### Durable-work entry and concurrent sessions

Durable work is a bounded project objective whose result must survive the current conversation: another Agent would need its objective, completion criteria, current position, or evidence to resume or verify it; it creates reusable research or conclusions; or it makes a meaningful multi-step project change. General questions, readily re-derived read-only checks, transient implementation notes, and atomic meaning-preserving edits such as an isolated typo or broken-link correction do not require this entry loop.

Before durable mutation, run Brief and establish a committed Work Item as the current session's Focus. If the human explicitly names an existing stable Work Item, that request may serve as the selection after its current authoritative state is reported. An inferred match is only a recommendation. If no Work Item fits, inspect read-only, search open and relevant terminal records, present update-existing, retain-candidate, consolidate-or-supersede, and create-new options, and stop before mutation. Work Item creation approval and external write authorization remain distinct even when one explicit human statement grants both.

After Focus is established, update only that session's selected non-terminal Work Item within its authority. Capture reusable Knowledge/Evidence references when the Profile permits Agent writes; otherwise propose the record. When Candidate intake requires human approval, present a draft instead of mutating it. At a useful boundary, propose a bounded checkpoint or clear without changing Handoff until confirmed.

Multiple Agent sessions may select different committed Work Items on the same branch or working tree. At session entry, observe the current revision, existing non-ignored dirty paths, and mapped Work Items as transient context, not canonical project state. Do not create a persistent Agent/session registry. A change outside the current session's Focus is not automatically divergence: classify it as limited parallel work when it fits another active Work Item, or as unattributed parallel work when its session origin is unavailable but that scope match is supported. If it fits no Work Item, report unscoped durable work. If concurrent scopes touch the same files, contracts, Decisions, or dependencies, report overlap or ambiguity and stop before a conflicting mutation. Focus divergence applies only between the current session's selected Work Item and work that can be reliably attributed to that session.
...
Maintain one stable authoritative Handoff source or location, not a continuously synchronized status record. It must explicitly contain either no current checkpoint or one bounded checkpoint. A checkpoint contains only the Focus of the session selected for transfer, the last state verified against authoritative sources, the exact next action, current blockers or unknowns, project-appropriate freshness evidence, and the minimum references needed to resume. It does not assert a project-wide Focus or replace other active Work Items; preserve their current position in their own authoritative records. Keep completed-work history, parallel-session state, Decision rationale, detailed verification evidence, and change logs in their authoritative Work Items, Decisions, evidence records, tracker, or version control rather than copying them into Handoff.
...
1. **Initialize**: inspect instructions, project records, trackers, Agent clients, permissions, Git availability, working-tree observation options, and concurrent-session expectations without mutation. Ask the human to choose local or external Work Item ownership, then propose area-by-area ownership modes, mappings, candidate intake, ready horizon, the Handoff source and explicit empty representation, freshness evidence, a minimal always-on durable-work entry hook, generated files, migrations, approval roles, conflicts, and verification. When no checkpoint exists, include creation of the explicit empty state in the proposal rather than fabricating checkpoint content. Generate, activate, or migrate only after human approval.
2. **Brief**: first show every committed open Work Item as a compact one-line map, distinguish blocked items and the bounded ready inventory, and recommend a next item with evidence. Exclude candidates and terminal records. Report a valid empty Handoff as `no current checkpoint`; otherwise verify the checkpoint against authoritative sources and report `verified current`, `stale`, or `unknown` before relying on its claims. Observe the current revision and pre-existing non-ignored dirty paths as transient context. After the human selects this session's Focus, load only that item's details, applicable Decisions, evidence, and any bounded checkpoint; label unverified checkpoint claims rather than presenting them as current facts.
3. **Enter durable work**: trigger before durable mutation or reusable research even when the user does not name Continuity. Accept an explicitly named existing stable Work Item as this session's human selection. When only an inferred match or no committed item exists, remain read-only, present the creation-gate options, and wait for selection and every required local or external write approval. After selection, update only that Work Item, capture reusable evidence according to its authority, and propose rather than silently perform Candidate or Handoff mutations that require confirmation.
4. **Record Decision**: apply the durable-impact threshold, create a proposed record, recognize an explicit human choice as approval, and maintain rejection and supersession without rewriting history.
5. **Handoff**: when the human explicitly requests a checkpoint or confirms a proposed checkpoint draft, update the authoritative Work Item as separately authorized and replace the Handoff representation with verified state and an exact resume point for that selected transfer. When the human requests or confirms a clear because no resume point remains, replace it with the explicit empty state. Keep checkpoints bounded, do not treat them as project-wide Focus, and do not turn them into a chronological or parallel-session archive. Without that request or confirmation, report that an existing checkpoint may become stale and leave the canonical representation unchanged.
6. **Prepare Verification**: before functional verification, ask the human to select `independent verification` or `change-informed regression verification`; use independent verification as the default only when an uncontaminated bounded context is possible. Independent verification may receive approved requirements, completion criteria, public interfaces, executable behavior, and observations gathered by the verifier, but not the current Handoff, development history, prior pass claims, implementation solution, or failure hypotheses. Change-informed regression verification may additionally receive the minimum change scope, risk summary, and stable Work Item identifiers, but not prior conclusions as evidence. If the Agent has already consumed excluded context and cannot restart cleanly, label the result `informed verification`. Record environment, procedure, observations, reproduction information, and context mode before summarizing the result into the Work Item or Handoff.
7. **Audit**: read all mapped and owned sources without mutation; in Git projects also inspect non-ignored tracked and untracked working-tree changes even when Handoff is empty. Report broken references, invalid transitions, duplicate or contradictory authority, lossy native-state mappings, ready-horizon violations, supersession errors, stale state, unscoped durable work, Focus divergence, parallel-work overlap, ungrounded claims, excessive context, and code/document/tracker conflicts with evidence and a proposed correction. Distinguish a valid explicit empty Handoff from a missing, actually blank, malformed, placeholder, mixed empty-and-checkpoint, or multiply authoritative representation, but do not stop other Audit checks after recognizing the empty state. Treat work matching another active Work Item as parallel rather than divergent unless it affects the checkpoint or overlaps the current session's files, contracts, Decisions, or dependencies. When meaningful dirty work has no selected or matching Work Item, return a non-clean `needs attention` warning rather than a clean or `OK` summary; whether warnings produce a non-zero process status remains a project adaptation. Never change Focus, priority, status, Handoff, or canonical content automatically.
...
- project and nested Agent instructions, including the smallest always-on hook that routes durable work into Brief;
...
- external write permissions, privacy constraints, Git availability, working-tree observation limits, concurrent Agent expectations, and existing validation tools;
...
- summary and detailed write-authority declarations for Candidate, Knowledge/Evidence, and Handoff, rejecting any summary that grants broader mutation authority than its detailed policy;
...
- proposed Skill names, triggers, non-triggers, responsibilities, paths, bundled resources, and the minimal always-on durable-work entry instruction;
...
- session-scoped Focus behavior, concurrent-session and working-tree overlap handling, and the absence of a persistent Agent/session registry;
- the single Handoff source, explicit empty representation, selected-transfer scope, checkpoint and clear authority, bounded checkpoint content, freshness evidence and startup behavior, native-state mappings, verification modes, and projection timestamp boundaries;
- one representative ordinary durable-work entry, Brief, concurrent-session, Decision, Handoff, Prepare Verification, and Audit scenario, including a non-trigger control;
...
- include meaningful project work about to begin without a Session Focus as a trigger, make general questions, readily re-derived checks, and atomic meaning-preserving edits explicit non-triggers, and keep detailed variants in progressively loaded references;
...
Confirm that the target Agent discovers the generated Skills and that Profile and record metadata pass the approved project-local schemas. Exercise an ordinary durable-work request that does not name Continuity, a non-trigger control, an empty-Handoff working-tree Audit, and concurrent sessions with distinct Work Items; verify semantic invariants separately from schema validity. Report mapped authoritative sources, created or migrated records, human decisions still required, and remaining uncertainty.
...
When an update is available, inspect the existing local implementation and compare the two exact Blueprint documents. Present a semantic migration proposal before writes. Preserve intentional local behavior, schema extensions, IDs, record history, and authority mappings. Explicitly assess Work Item ownership, candidate intake and creation gates, ready horizon, completion review, the always-on durable-work entry hook, generated Skill triggers and non-triggers, session-scoped versus project-wide Focus, concurrent-session handling, working-tree Audit behavior, cross-representation authority consistency, the single Handoff source, explicit empty representation, selected-transfer scope, checkpoint and clear authority, bounded checkpoint content, startup behavior, native-state mappings, verification context, and readable YAML impact when those semantics differ. Classify the existing Handoff as a confirmed checkpoint, placeholder or fabricated summary, history-heavy record, or unresolved content. Preserve and freshness-check a confirmed checkpoint. Propose converting a placeholder or fabricated summary to the explicit empty state. For a history-heavy record, identify authoritative homes for every unique fact before proposing a bounded checkpoint or empty state. Treat existing dirty work, a global Focus representation, and missing Evidence or Candidate records as unresolved migration inputs: propose their authoritative Work Item or disposition, but never backfill, reassign, delete, clear, or reinterpret them automatically. Include conditional schema validation, Brief and Audit behavior, external writes, failure handling, ordinary durable-work entry, non-trigger, concurrent-session, keep-checkpoint, clear-checkpoint, and empty-Brief verification, and rollback. Update the Installation Receipt and every Skill provenance together only after all approved changes and local verification succeed. If verification fails or the migration is partial, keep the prior revision in the Receipt and provenance. If the installation predates receipts, propose creation of one from existing exact provenance before migration. Never regenerate or overwrite automatically.
...
- The human selects the Focus for each Agent session after the first-stage Brief; an explicit request naming an existing stable Work Item is a selection for that session, while an inferred match is not. Concurrent sessions may select different Work Items.
...
- Creating a project-wide Session Focus, persistent Agent/session registry, or mandatory branch/worktree per concurrent Agent.
...
- multiple open Work Items and concurrent sessions can be summarized without forcing one project-wide active objective or Focus;
...
- durable work that does not name Continuity enters Brief before mutation, while documented non-triggers avoid unnecessary overhead;
- a two-stage Brief ends with a human-selected session-scoped Focus and bounded detailed Context, and an inferred semantic match cannot select it;
...
- the single checkpoint transfers one selected session without replacing other active Work Items, and parallel resume state remains in authoritative Work Items rather than a session archive;
...
- a new session reports a valid empty state as `no current checkpoint`, or verifies material checkpoint claims against authoritative sources and reports `verified current`, `stale`, or `unknown` before relying on them; empty-state freshness does not skip working-tree, unscoped-work, or authority Audit;
- concurrent sessions may select different Work Items on the same branch; non-overlapping work is classified as parallel, unattributed work is not falsely assigned, and overlapping files, contracts, Decisions, or dependencies produce a non-clean ambiguity warning before conflicting mutation;
